		...
		***/
        ...
        if ($validator->passes()) {
            ...
            $shuzu=[
                ...
                '箱长（米）' =>['N','Y','20','wms_length'],
                '箱宽（米）' =>['N','Y','20','wms_wide'],
                '箱高（米）' =>['N','Y','20','wms_high'],
...
//                    $list['wms_out_unit']       = $v[4];
    ...
    public function getGood(Request $request){
        ...
        //dd($where);
        $data['info']=ErpShopGoodsSku::where($where)->select('self_id','external_sku_id','good_name','wms_spec')->get();
        $msg['code']=200;
        $msg['msg']="数据拉取成功";
        $msg['data']=$data;
