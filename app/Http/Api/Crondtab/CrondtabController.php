...
//                $dispatch_list = array_column($tmsOrderDispatch->toArray(),'self_id');
    ...
    public function cancelPayOrder(Request $request){
        ...
        $select = ['self_id','order_status','total_money','pay_type','group_code','group_name','total_user_id','order_type','on_line_flag','gather_time','order_id','update_time'];
        $select1 = ['self_id','order_status','total_money','pay_type','group_code','group_name','total_user_id','order_type','gather_time','total_money','update_time'];
        $order_list = TmsOrderDispatch::where($where)->select($select)->get();
        foreach ($order_list as $k => $v) {
            if ($now_time > strtotime($v->gather_time)) {
                $order = TmsOrder::where('self_id',$v->order_id)->select($select1)->first();
